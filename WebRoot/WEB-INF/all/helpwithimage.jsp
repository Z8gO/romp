...
<div class="main">
      ...
      <div class="mainbar">
       ...
       <!-- 放在article  这个div里面-->
        ...
        <c:if  test="${4==user.iid}">        	
        	<li><a href="GetQuestion?imgid=${question.imgid}"  style="text-align: center;text-decoration: none;color: red;">我认识它，我来补充!</a></li>
        </c:if>
        	<!-- c:if end-->
        ...
        <!-- NotPassQue?pc=1&uid=${sessionScope.user.uid} -->
        		<a href="NotPassQue?pc=${sessionScope.pageBean.pc-1}&uid=${sessionScope.user.uid}" > 上一页</a>
        </c:if>
		<c:if test="${sessionScope.pageBean.pc<sessionScope.pageBean.tp}">
        		<a href="NotPassQue?pc=${sessionScope.pageBean.pc+1}&uid=${sessionScope.user.uid}" > 下一页</a>
        </c:if>
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="NotPassQue?pc=${i}&uid=${sessionScope.user.uid}" >${i }</a>
   	  </c:otherwise>
   </c:choose> 	
 </c:forEach>
