...
<div class="main">
  <div class="header">
    <div class="header_resize">
      ...
      <h2>Manage <span>New</span> Question</h2>
      <p>这些都是大家提出的问题，请您认真审核！ </p>
      <div class="clr"></div>
      </div> 
    </div>
  </div>
      ...
      <div class="mainbar">
       ...
       <!-- 放在article  这个div里面-->
        <c:forEach  items="${sessionScope.pageBean.list}"  var="question">
        	...
        	<li>　</li>
        	<li>上传者ID：${question.uid}</li>
        	
        	<!-- ******************** -->
        	<li><a href="PassQuestion?imgid=${question.imgid}"  style="text-align: center;text-decoration: none;color: red;text-align: center;" onclick="return  pass();">通过审核</a></li>
        </ul>
        </div>
        </c:forEach>
         </div>
        ...
        <c:if test="${sessionScope.pageBean.pc>1}">
        		<a href="FindBlockedQuestion?pc=${sessionScope.pageBean.pc-1}" > 上一页</a>
        </c:if>
		<c:if test="${sessionScope.pageBean.pc<sessionScope.pageBean.tp}">
        		<a href="FindBlockedQuestion?pc=${sessionScope.pageBean.pc+1}" > 下一页</a>
        </c:if>
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="FindBlockedQuestion?pc=${i}" >${i }</a>
   	  </c:otherwise>
   </c:choose> 	
 </c:forEach>
      ...
      <div class="sidebar">
        <div class="gadget">
          ...
          <ul class="sb_menu">
            ...
            <li><a href="javascript:void(0)">审核用户提交的问题</a></li>
