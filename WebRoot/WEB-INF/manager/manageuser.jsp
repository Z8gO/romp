...
<div class="main">
    ...
    <div class="content_resize"  style="width: 990px;">
      <div class="mainbar">
        <div class="article"  style="width: 720px;">
          <h2><span>Manager</span> Activate User</h2>
          <div class="clr"></div>
          <p>冻结某个账号后，请在管理冻结账号页面查看！</p>
       <!-- 放在article  这个div里面-->
       <table  id="usertable">
       <tr><th>用户ID</th>
       <th>用户名</th>
       ...
       <th>操作</th>
        ...
        <c:forEach  items="${sessionScope.userSuccessPageBean.list}"  var="users">
 			...
 			<tr>
				...
				<td>
					<c:if  test="${users.uname==user.uname}">
						<a  href="javascript:void(0)">本账号</a>
					</c:if>			
					<c:if  test="${users.uname!=user.uname}">
						<a  href="UpdateUserStatus?uid=${users.uid}"  onclick="return  blockuser();">冻结账号</a>
					</c:if>		
				</td>
 				</tr>
    	...
    	<a href="javascript:void(0)" > 共${sessionScope.userSuccessPageBean.tp}页</a>
        <a href="javascript:void(0)" > 当前是第${sessionScope.userSuccessPageBean.pc}页</a>  	
        <c:if test="${sessionScope.userSuccessPageBean.pc>1}">
        		<a href="FindSuccessUser?pc=${sessionScope.userSuccessPageBean.pc-1}" > 上一页</a>
        </c:if>
		<c:if test="${sessionScope.userSuccessPageBean.pc<sessionScope.userSuccessPageBean.tp}">
        		<a href="FindSuccessUser?pc=${sessionScope.userSuccessPageBean.pc+1}" > 下一页</a>
        </c:if>
        
       <c:choose>
 	<c:when test="${sessionScope.userSuccessPageBean.tp <= 6 }">
 		<c:set var="begin" value="1"/>
 		<c:set var="end" value="${sessionScope.userSuccessPageBean.tp}"/>
 	</c:when>
 	<c:otherwise>
 		<c:set var="begin" value="${sessionScope.userSuccessPageBean.pc-2 }"/>
 		<c:set var="end" value="${sessionScope.userSuccessPageBean.pc + 3}"/>
 		<c:if test="${begin < 1 }">
 		  <c:set var="begin" value="1"/>
 		  <c:set var="end" value="6"/>
 		</c:if>
 		<c:if test="${end > sessionScope.userSuccessPageBean.tp }">
 		  <c:set var="begin" value="${sessionScope.userSuccessPageBean.tp-5 }"/>
 		  <c:set var="end" value="${sessionScope.userSuccessPageBean.tp }"/>
 		</c:if> 		
 	</c:otherwise>
 </c:choose>
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  <c:when test="${i eq sessionScope.userSuccessPageBean.pc }">
 ...
 <c:if test="${end < sessionScope.userSuccessPageBean.tp }">
      ...
      <div class="sidebar">
        <div class="gadget">
          ...
          <div class="clr"></div>
          <ul class="sb_menu">
           <li><a href="javascript:void(0)">管理正常状态的用户</a></li>
            <li><a href="FindBlockedUser?pc=1">审核冻结状态的用户</a></li>
            <li><a href="FindBlockedQuestion?pc=1">审核用户提交的问题</a></li>
